...
"""
        ...
        fd.write("          '        y = x.applyfunc(lambda a: sp.%s(a))\\n'  ...\n" % d['spname'])
        fd.write("          '    else:\\n'   ...\n" )
        fd.write("          '        y = sp.%s(x)\\n'  ...\n" % d['spname'])
        fd.write("          '    return (y,)\\n' ];\n")
        fd.write("\n");
...
def make_unit_tests(L):
    ...
    for d in L:
        ...
        if d['out_val_from_oct']:
            ...
            fd.write("  f2 = %s;\n" % d['test_out_val'])
        fd.write("  r(1) = abs(double(f1) - f2) < 1e-15;\n\n")
        ...
        fd.write("  r(2) = all(all( abs(double(f1) - f2) < 1e-15 ));\n\n")
...
def output_for_gitignore(L):
    ...
    for d in L:
        f = d['name']
        print "inst/unittests/test_autogen_%s.m" % f
