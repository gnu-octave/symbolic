function obj = check_and_convert(var_name)
  ...
  pyexec(strjoin({['if isinstance(', var_name, ', sp.Matrix) and ', var_name, '.shape == (1, 1):'],
                  ['  ', var_name, ' = ', var_name, '[0, 0];']}, newl));

  str_check_is_list = strjoin({['if isinstance(', var_name, ', (list, tuple)):'],
                              '  is_list = True',
                              'else:',
                              '  is_list = False'},newl);
  ...
  for i=1:n
    ...
    if is_list_curvar
      ...
      str_check_is_sym = strjoin({['if ', cur_var, ' is None or isinstance(', cur_var, ', (sp.Basic, sp.MatrixBase)):'],
                                  '  is_sym = True',
                                  'else:',
                                  '  is_sym = False'},newl);
